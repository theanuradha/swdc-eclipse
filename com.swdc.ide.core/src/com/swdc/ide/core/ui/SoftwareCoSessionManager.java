...
public class SoftwareCoSessionManager {
	...
	public void storePayload(String payload) {
		...
		try {
			...
			SWCoreLog.logErrorMessage("Software.com: Error appending to the Software data store file");
	    ...
	    if (f.exists()) {
		    	...
		    	try {
		    		...
		    		if (sb.length() > 0) {
		    			...
		    			SWCoreLog.logInfoMessage("Software.com: No offline data to send");
		    					    		}
		    	} catch (Exception e) {
		    		SWCoreLog.logErrorMessage("Software.com: Error trying to read and send offline data.");
	...
	public static void setItem(String key, String val) {
	    ...
	    try {
	    	...
	    	SWCoreLog.logErrorMessage("Software.com: Failed to write the key value pair (" + key + ", " + val + ") into the session.");
            SWCoreLog.logException(e);
	...
	private static JsonObject getSoftwareSessionAsJson() {
	    ...
	    if (f.exists()) {
	    		try {
	    			...
	    			SWCoreLog.logErrorMessage("Software.com: Error trying to read and json parse the session file.");
                    SWCoreLog.logException(e);
	...
	private static HttpResponse makeApiCall(String api, boolean isPost, String payload) {
		...
		if ( response != null ) {
			...
			try {
				...
				SWCoreLog.logErrorMessage("Software.com: Unable to get the response from the http request.");
				SWCoreLog.logException(e);
	...
	protected static class SessionManagerHttpClient implements Callable<HttpResponse> {
		...
		public HttpResponse call() throws Exception {
			...
			try {
				...
				SWCoreLog.logErrorMessage("Software.com: Unable to make api request.");
				SWCoreLog.logException(e);
