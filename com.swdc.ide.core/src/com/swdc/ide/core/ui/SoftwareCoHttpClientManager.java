			...
			public void run() {
				...
				if ( response != null ) {
					...
					try {
						...
						if (httpResponse != null) {
							...
							if (httpResponse.getEntity() != null) {
								try {
									...
									SWCoreLog.error("Software.com: Unable to parse the non-null plugin manager response.", e);
								}
							}
							...
							if (responseStatus >= 300) {
								SWCoreLog.logErrorMessage("Software.com: Unable to send the keystroke payload, "
						...
						SWCoreLog.error("Software.com: Unable to get the response from the http request.", e);
						isRequestOk = false;
					}
				}
	...
	***/
	protected class KeystrokeDataSendTask implements Callable<HttpResponse> {
		...
		public HttpResponse call() throws Exception {
			...
			try {
				...
				SWCoreLog.error("Software.com: Unable to send the keystroke payload request.", e);
			} finally {
				if (request != null) {
	...
	protected class DownloadPMHandler implements Callable<Void> {
		...
		public Void call() throws Exception {
	        ...
	        SWCoreLog.logInfoMessage("Downloading Software plugin manager");
	        ...
	        try {
	            ...
	            fos.close();
	            SWCoreLog.logInfoMessage("Completed download of the Software plugin manager");
	            Desktop.getDesktop().open(new File(saveAs));
	        } catch (Exception e) {
	        	SWCoreLog.error("Failed to download the Software plugin manager, reason: " + e.toString(),e);
	        }
	        downloadingPM = false;
